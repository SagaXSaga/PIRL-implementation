...
def train_one_epoch(model, loader, criterion, optimizer, epoch, device):
    ...
    for images, labels in tqdm(loader, desc=f"Epoch {epoch}"):
        ...
        # Apply Mirror_Detection transform to get both original and flipped images
        images, flipped_images = Mirror_Detection(flip_prob=0.5, return_image=True)(images)
        # Forward pass for both original and flipped images
        outputs, transformed_outputs = model(images, transformed_x=flipped_images)
        optimizer.zero_grad()
